...
6. **Deferred to v0.1 #6 — Reactive substrate end-to-end.** The convention is substrate-agnostic — the six rules don't care whether an agent's claim ritual was triggered by a cron tick or a webhook event. v0.0.1 verified the cron substrate end-to-end (T7.1, T6.4 MVP) and shipped `docs/routines/reactive-setup.md` documenting the target configuration, but did not run the reactive substrate against a live GitHub webhook delivery. The v0.0.1 probe established that one of the three candidate paths is closed: (path 1) github tool's `handle_webhook` action — viable in principle once F18 (WASM ABI mismatch) cleared, but requires the agent to be invoked with the webhook payload already in context, which itself depends on path 2 or path 3 for inbound delivery; (path 2) reactive routine with `trigger_type: webhook` and per-routine `/hooks/<path>` endpoint — **empirically closed.** `routine_create(trigger_type=webhook)` accepts the call and stores the routine, but the daemon registers no corresponding HTTP route. Probe: `POST https://<host>/hooks/<routine-path>` returns 404; daemon startup logs show zero `Registered ... endpoint` lines for webhook-trigger routines. IronClaw's own docs note this as _"not yet exposed in the web gateway UI"_; the runtime registration is not implemented either, at least in v0.28.0; (path 3) IronClaw's HTTP webhook channel (`POST /webhook` with `{user_id, message}` body and `X-Webhook-Secret` header) plus an external adapter (Cloudflare Worker or small script) that translates GitHub's payload shape (`{action, issue, ...}`) to IronClaw's expected shape — the only viable approach. v0.1 enables the HTTP webhook channel, designs and deploys the GitHub→IronClaw adapter (Cloudflare Worker fronting `ironclaw.multiagency.services/webhook` is the recommended shape since the cloudflared tunnel is already in place), wires GitHub webhook delivery against `MultiAgency/test` or a fork, verifies the reactive routine fires within seconds of a `ready`-label event, and codifies the chosen adapter in the kanban-worker SKILL.md plus README.
