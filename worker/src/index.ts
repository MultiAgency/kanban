...
function translateToPrompt(payload: GitHubPayload): string | null {
  ...
  return [
    ...
    `   Each argument:`,
    `   - action: literal "create_or_update_file"`,
    `   - owner: literal "${owner}"`,
    `   - repo: literal "${repo}"`,
    `   - path: the path string you parsed from the issue body in step 1`,
    `   - message: a one-line commit message in the shape "docs: add (the parsed path written out verbatim) per #${issue.number}". For an imaginary parsed path docs/adr/0042-multi-tenancy.md the message would be exactly: docs: add docs/adr/0042-multi-tenancy.md per #${issue.number}. Construct yours analogously by writing the actual parsed path string into the message.`,
    `   - content: the markdown text you drafted in step 4, written verbatim`,
    `   - branch: literal "main"`,
    `   Worked example call for an imaginary parsed path docs/adr/0042-multi-tenancy.md — substitute your actual parsed path for both occurrences of that imaginary path:`,
    `       github(action=create_or_update_file, owner=${owner}, repo=${repo}, path=docs/adr/0042-multi-tenancy.md, message="docs: add docs/adr/0042-multi-tenancy.md per #${issue.number}", content=(the markdown drafted in step 4), branch=main)`,
    `   In the actual call, the content argument is the literal markdown body — not the parenthesized prose "(the markdown drafted in step 4)". That parenthesized phrase appears only in this prompt as a description.`,
    ...
    `   The issue_number argument is ${issue.number}. The body argument is a markdown string you compose now. Its structure: the literal text \`**Handoff:**\` followed by a one-paragraph summary of what was done, a blank line, a fenced block opened with three backticks plus the word \`handoff\` (no space), a JSON object with at least one of changed_files (array of path strings), verification (array of step strings), residual_risk (array of risk strings), or links (array of URL strings), then the closing three-backtick fence.`,
    `   Worked example body for an imaginary multi-tenancy ADR fire — your actual body is composed for the actual issue's deliverables:`,
    ...
    `       \`\`\``,
    `   Call shape:`,
    `       github(action=create_issue_comment, owner=${owner}, repo=${repo}, issue_number=${issue.number}, body=(your composed markdown body))`,
    `   In the actual call, the body argument is the literal markdown — not the parenthesized phrase "(your composed markdown body)" which is only a description here.`,
    ``,
