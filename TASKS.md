...
- [ ] **T8.4: Wire GitHub webhook against `MultiAgency/test` (or fork) + verify end-to-end**
  ...
  - Status: transport verified (T8.2 + T8.3 banked), dispatch unverified. `routine_runs` records zero `trigger_type=webhook` fires; the existing `kanban-test` webhook routine has placeholder config (`prompt="echo"`, `trigger_config={"path":null,"secret":null}`) so Worker forwards arrive at IronClaw but match no routine. Closing this loop is a v0.1.x work item — either fix `kanban-test`'s `trigger_config` + `prompt` to match what the Worker forwards, or replace with a fresh webhook routine. See Phase 9 honesty correction in `docs/ironclaw-tracer-outcome.md` and the v0.1.1 CHANGELOG framing.
