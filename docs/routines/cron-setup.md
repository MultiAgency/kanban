...
- **Finding 26 — placeholder-leak cruft.** The cron prompt embedded literal tool-call templates with placeholder markers for the agent to substitute into; the agent often emitted a transformation of the marker instead, committing files at placeholder paths. The fix is the prose-with-worked-example prompt structure in `cron-tick-prompt.md` (an earlier distinctive-marker form did not lower the rate); `.github/workflows/no-cruft.yml` is the CI backstop, rejecting any commit whose changed paths contain `<` or `>`.
...
- **Routine completes but the issue is half-claimed (assignee set, label unchanged).** Plausibly Finding 30 — the http-tool label-add call's body landed under an unrecognized parameter key, so GitHub received an empty body and rejected the call. Manual cleanup: either complete the label swap by hand and let the agent finish, or revert the assignee and re-queue.
